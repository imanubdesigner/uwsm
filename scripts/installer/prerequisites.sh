...
run_command "pacman -S --noconfirm ufw firefoxpwa wf-recorder curl cmake ninja bat bat-extras yazi firefox obsidian neovim discord mpv yt-dlp spotify-launcher cava evince zsh zsh-completions zoxide fzf eza kitty imagemagick starship nano fastfetch ripgrep rofi btop pacman-contrib tar xarchiver p7zip unrar unzip thunar tumbler gvfs gvfs-mtp udisks2 xdg-user-dirs thunar-archive-plugin thunar-volman ffmpegthumbnailer ffmpeg poppler-glib librsvg" "Install complete application suite (browsers, media, development, system tools)" "yes"
