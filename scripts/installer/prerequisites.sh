...
run_command "pacman -S --noconfirm firefox obsidian neovim discord mpv yt-dlp spotify-launcher cava switcheroo evince zsh zsh-completions zoxide fzf eza kitty imagemagick starship nano fastfetch ripgrep rofi btop pacman-contrib tar file-roller p7zip unrar unzip thunar tumbler gvfs gvfs-mtp udisks2 xdg-user-dirs thunar-archive-plugin thunar-volman ffmpegthumbnailer ffmpeg poppler-glib librsvg" "Install complete application suite (browsers, media, development, system tools)" "yes"

# -------------------- Configuration Files --------------------
run_command "\
cp -r $BASE_DIR/configs/cava /home/$SUDO_USER/.config/ && \
cp -r $BASE_DIR/configs/fastfetch /home/$SUDO_USER/.config/ && \
cp -r $BASE_DIR/configs/kitty /home/$SUDO_USER/.config/ && \
cp -r $BASE_DIR/configs/rofi /home/$SUDO_USER/.config/ && \
cp -r $BASE_DIR/configs/btop /home/$SUDO_USER/.config/ && \
chown -R $SUDO_USER:$SUDO_USER /home/$SUDO_USER/.config/cava && \
chown -R $SUDO_USER:$SUDO_USER /home/$SUDO_USER/.config/fastfetch && \
chown -R $SUDO_USER:$SUDO_USER /home/$SUDO_USER/.config/kitty && \
chown -R $SUDO_USER:$SUDO_USER /home/$SUDO_USER/.config/rofi && \
chown -R $SUDO_USER:$SUDO_USER /home/$SUDO_USER/.config/btop && \
find /home/$SUDO_USER/.config/rofi -name '*.sh' -type f -exec chmod +x {} +" \
  "Copy all configuration files (cava, fastfetch, kitty, rofi, btop)" "yes" "no"
