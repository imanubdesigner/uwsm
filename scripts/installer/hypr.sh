...
run_command "pacman -S --noconfirm hyprland wtype wf-recorder swappy jq grim slurp wl-clipboard libnotify cliphist waybar hyprpicker hypridle hyprlock uwsm xdg-desktop-portal-gtk xdg-desktop-portal-hyprland qt5-wayland qt6-wayland hyprsunset hyprpolkitagent dunst" "Install complete Hyprland ecosystem (core, session manager, wayland support, addons)" "yes"
...
run_command "cp -r $BASE_DIR/configs/swappy /home/$SUDO_USER/.config/ && chown -R $SUDO_USER:$SUDO_USER /home/$SUDO_USER/.config/swappy" "Copy Swappy config folder (Must)" "yes" "no"
echo "------------------------------------------------------------------------"
