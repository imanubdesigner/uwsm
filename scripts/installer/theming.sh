...
run_command "echo 'manu ALL=(ALL) NOPASSWD: /usr/bin/cpupower, /usr/bin/tlp, /usr/bin/ufw, /usr/bin/systemctl, /usr/bin/snapper, /usr/bin/btrfs, /usr/bin/sed, /usr/bin/limine-update, /usr/bin/tee, /usr/bin/chmod' | sudo tee /etc/sudoers.d/manu-powerprofile && sudo chmod 0440 /etc/sudoers.d/manu-powerprofile" "Configure passwordless sudo" "no" "no"
