...
run_command "cp $BASE_DIR/assets/tlp.conf /etc/tlp.conf" "Copy TLP configuration" "no"
run_command "echo 'manu ALL=(ALL) NOPASSWD: /usr/bin/cpupower, /usr/bin/tlp, /usr/bin/ufw, /usr/bin/systemctl, /usr/bin/snapper, /usr/bin/btrfs, /usr/bin/sed, /usr/bin/limine-update, /usr/bin/tee' | tee /etc/sudoers.d/manu-powerprofile && chmod 0440 /etc/sudoers.d/manu-powerprofile" "Configure passwordless sudo for first-run scripts" "no"
...
run_command "cp $BASE_DIR/assets/pacman.conf /etc/pacman.conf" "Copy pacman.conf to /etc for manu user" "no"
