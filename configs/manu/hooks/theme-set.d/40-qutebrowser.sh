...
theme_dir="$config_dir/manu"
draw_file="$theme_dir/draw.py"
config_file="$config_dir/config.py"
light_file="$HOME/.config/manu/current/theme/light.mode"
...
# Ensure config.py imports the manu theme
...
manu.draw.apply(c)
PYEOF
elif ! grep -q 'manu.draw' "$config_file"; then
    sed -i '1i import manu.draw' "$config_file"
    echo 'manu.draw.apply(c)' >> "$config_file"
fi
