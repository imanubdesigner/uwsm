...
# Typora theme hook - generates CSS with current manu theme colors

new_typora_file="$HOME/.config/manu/current/theme/typora.css"
...
if [ -f "$new_typora_file" ]; then
    cp -f "$new_typora_file" "$typora_dir/manu.css"
else
    create_dynamic_theme
    cp -f "$new_typora_file" "$typora_dir/manu.css"
fi
