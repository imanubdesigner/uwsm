...
output_file="$HOME/.config/manu/current/theme/steam.css"
...
font_path=$(fc-list $(manu-font-current) file | grep -ioP '.*\.ttf' | head -n 1)
...
modify_steam_theme() {
    if [[ ! -d "$adwaita_location/adwaita/colorthemes/manu" ]]; then
        mkdir $adwaita_location/adwaita/colorthemes/manu/
    fi
}
modify_install_script() {
    if ! grep -q "manu" "$adwaita_location/install.py"; then
        sed -i.bak 's/\("cantarell"\)/\1, "manu"/' $adwaita_location/install.py
    fi
}
...
cp -p -f "$output_file" "$adwaita_location/adwaita/colorthemes/manu/manu.css"
