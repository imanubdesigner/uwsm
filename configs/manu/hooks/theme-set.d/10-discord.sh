...
output_file="$HOME/.config/manu/current/theme/vencord.theme.css"
possible_paths=(
...
check_for_theme() {
    if [[ -f $HOME/.config/manu/current/theme/vencord.theme.css ]]; then
        for path in "${possible_paths[@]}"; do
            if [ -d "$path" ]; then
                if [[ -f "$path/vencord.theme.css" ]]; then
                    rm "$path/vencord.theme.css"
                fi
                cp -f $HOME/.config/manu/current/theme/vencord.theme.css "$path/vencord.theme.css"
            fi
