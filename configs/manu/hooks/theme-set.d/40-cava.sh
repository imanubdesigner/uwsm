...
theme_template="$HOME/.config/manu/current/theme/cava_theme"
...
mkdir -p "$theme_dir"
cp -f "$theme_template" "$theme_dir/manu"

config_file="$HOME/.config/cava/config"
if [ -f "$config_file" ] && ! grep -q "theme = 'manu'" "$config_file"; then
    sed -i "/^\[color\]/a theme = 'manu'" "$config_file"
fi
