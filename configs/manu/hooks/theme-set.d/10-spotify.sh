...
create_spicetify_styling() {
    mkdir -p "$HOME/.config/spicetify/Themes/manu"
cat > "$HOME/.config/spicetify/Themes/manu/user.css" << EOF
:root,
.encore-dark-theme,
.encore-base-set,
...
change_spicetify_theme() {
    spicetify config current_theme manu > /dev/null
...
cat > "$HOME/.config/spicetify/Themes/manu/color.ini" << EOF
[base]
main                = ${color00}
player              = ${color00}
card                = ${color00}
