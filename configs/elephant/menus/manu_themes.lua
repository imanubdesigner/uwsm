--
-- Dynamic Hypruccin Theme Menu for Elephant/Walker
--
Name = "manuthemes"
NamePretty = "Manu Themes"
...
function GetEntries()
	local entries = {}
	local user_theme_dir = os.getenv("HOME") .. "/.config/manu/themes"
	local manu_path = os.getenv("MANU_PATH") or ""
	local default_theme_dir = manu_path .. "/themes"
			...
			if theme_name and not seen_themes[theme_name] then
				...
				if preview_path and preview_path ~= "" then
					...
					table.insert(entries, {
						...
						PreviewType = "file",
						Actions = {
							activate = "manu-theme-set " .. theme_name,
						},
					})
				end
			end
